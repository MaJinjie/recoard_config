...
__fzf_comprun() {
  if [[ "$(type _fzf_comprun 2>&1)" =~ function ]]; then
    ...
    if [ -n "${FZF_TMUX_OPTS-}" ]; then
      fzf-tmux ${(Q)${(Z+n+)FZF_TMUX_OPTS}} -- "$@" || touch $C_FZF_TMP_FAILURE/$$
    else
      fzf-tmux -d ${FZF_TMUX_HEIGHT:-40%} -- "$@" || touch $C_FZF_TMP_FAILURE/$$
    fi
  else
    shift
    fzf "$@" || touch $C_FZF_TMP_FAILURE/$$
  fi
}
...
_fzf_complete() {
  ...
  args=("$@")
  #print '$@'=$@ > ~/completion
  #print argslen=$#args args=$args >> ~/completion
  sep=
  for i in {0..${#args[@]}}; do
    if [[ "${args[$i]-}" = -- ]]; then
      sep=$i
  ...
  #print str_arg=$str_arg rest=$rest args=$args >> ~/completion
  ...
  #print lbuf=$lbuf cmd=$cmd post=$post >> ~/completion
  ...
  command rm -f "$fifo"
  print $matches > $C_FZF_TMP_RESULT/$$
  if [ -n "$matches" ]; then
